...
class Currency_rate_update_service(osv.osv):
    ...
    _columns = {
                    ...
                    'service' : fields.selection(
                                                    ...
                                                    ('UY_BCU_getter', 'Banco Central del Uruguay'),
                                                    ],
                                                    "Webservice to use",
                                                    required=True
                                                ),
...
class UY_BCU_getter(Curreny_getter_interface) :
    """Implementation of Currency_getter_factory interface
    for Banco Central del Uruguay - BCU service"""

    def rate_retrieve(self, dom, ns, curr) :
        """ Parse a dom node to retrieve-
        currencies data NO SE USA"""
        res = {}
        xpath_rate_currency = "/def:wechselkurse/def:devise[@code='%s']/def:kurs/text()" % (curr.lower())
        xpath_rate_ref = "/def:wechselkurse/def:devise[@code='%s']/def:waehrung/text()" % (curr.lower())
        res['rate_currency'] = float(dom.xpath(xpath_rate_currency, namespaces=ns)[0])
    ...
    def get_rate_from_txt(self, s, moneda):
        compra = 0
        for line in s:
          if line == "<TITLE>404 Not Found</TITLE>\n": 
              ...
              break
          strLista = line.split()
          if "ESTADOS" in strLista and "UNIDOS" in strLista and "DLS." in strLista and "USA" in strLista and "BILLETE" in strLista:
              #Tengo la lina con la cotización del dolar
              compra = strLista[7].replace(",",".")
              venta = strLista[8].replace(",",".") 
              break
        # Leo las lineas y busco el codigo de la moneda
        return float(compra)

    def get_updated_currency(self, currency_array, main_currency, max_delta_days):
        """implementation of abstract method of Curreny_getter_interface, formato url http://www3.bcu.gub.uy/autoriza/opinme/oicot100212.txt"""
        url_base = 'http://www3.bcu.gub.uy/autoriza/opinme/oicot'
        dia = datetime.strftime(datetime.today(), '%d')
        mes = datetime.strftime(datetime.today(), '%m')
        anio = datetime.strftime(datetime.today(), '%y')
        
        #we do not want to update the main currency
        if main_currency in currency_array :
            currency_array.remove(main_currency)
        # busca el archivo correspondiente al día actual
        _logger.info("currency_rate_update: Servicio de actualización BCU : obteniendo fuentes...")
        
        rate_date = datetime.strftime(datetime.today(), '%Y') + '-' + mes + '-' + dia
        rate_date_datetime = datetime.strptime(rate_date, '%Y-%m-%d')
        self.check_rate_date(rate_date_datetime, max_delta_days)
        #we dynamically update supported currencies        
        self.supported_currency_array.append('UYU')
        self.supported_currency_array.append('EUR')
        ...
        restar = 0
        while True:
            intDia = int(dia) + restar
            if intDia <= 9:
                dia = "0" + str(intDia)
            else:
                dia = str(intDia)
                
            url = url_base + dia + mes + anio + '.txt'
            s = self.get_url2(url)
            # Leo el resultado.
            #_logger.info("currency_rate_update: Supported currencies = " + str(self.supported_currency_array))
            ...
            for curr in currency_array :
                self.validate_cur(curr)
               # rate = main_rate * curr_data['rate_ref'] / curr_data['rate_currency']
                rate = self.get_rate_from_txt(s, curr)
                if rate == -1:
                  break
                else:  
                    if rate != 0:
                        valor = 1 / rate
                    self.updated_currency[curr] = valor
                    _logger.info("currency_rate_update: Rate retrieved : 1 " + main_currency + ' = ' + str(valor) + ' ' + curr)
                    cantidad = 0
                    break
            if rate == -1:
                  restar = -1
